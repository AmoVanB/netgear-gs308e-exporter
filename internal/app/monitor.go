...
// MonitorSwitch fetches port stats and status from a given switch
// every 'interval' seconds and accordingly update the metrics
...
func updatePortStats(config config.SwitchConfig, cookie string) error {
	...
	for i, node := range doc.Find(".portID").Nodes {
		...
		docFromNode.Find("input").Each(func(j int, selection *goquery.Selection) {
			...
			case "rxPkt":
				metrics.Var.ReceivedBytes.With(labels).Set(float64(val.Uint64()))
			case "txpkt":
				metrics.Var.TransmittedBytes.With(labels).Set(float64(val.Uint64()))
			case "crcPkt":
				metrics.Var.CRCErrorPackets.With(labels).Set(float64(val.Uint64()))
			default:
		...
		docFromNode.Find("input").Each(func(j int, selection *goquery.Selection) {
			...
			case 2:
				...
				if selection.AttrOr("value", "Down") == "Up" {
					metrics.Var.PortStatus.With(labels).Set(1)
				} else {
					metrics.Var.PortStatus.With(labels).Set(0)
				}
			case 4:
				...
				if err != nil {
					...
					metrics.Var.PortSpeed.With(labels).Set(0)
				} else {
					metrics.Var.PortSpeed.With(labels).Set(float64(speed))
				}
			default:
